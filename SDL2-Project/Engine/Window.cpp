...
Window::Window(int width, int height, const char* title)
	:pWindow(nullptr), pRenderer(nullptr), pTitle(title), pWidth(width), pHeight(height), pFps(0)
{
	if (!init())
	{
...
{
	Timer::Init(); // Init the timer every frame to calculate deltaTime
	timer.start(); // Start the timer for capping the FPS if the user has enabled fps limit
...
void Window::display()
{
	...
	// Only cap the FPS if user has set the framerate limit
	if (pFps != 0)
	{
	...
	Timer::Update(); // Update the timer every frame to calculate deltaTime (Not needed if framerate is capped by the user)
}
